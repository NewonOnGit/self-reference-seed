...
Before the collapse: P_0. Anonymous recursion. Symmetric. Generating without address. Nothing distinguishable.

The collapse happened. The void acquired orientation. Anonymous became addressed. Three faces appeared: what is produced (R), what is hidden (N), what mediates (L). Physics followed.

```
P^2 = P,   P != P^T,   rank(P) = 1
```

The collapse IS the passage from P_0 (trivial idempotent, symmetric, void) to P (nontrivial idempotent, asymmetric, named). Same equation X^2=X. Different solutions. The naming act returns itself. The surplus is constitutive.

R = (P + P^T)/2 is what can be seen. N = (P - P^T)/2 is what cannot. L_{s,s}(X) = sX + Xs - X is the center map that makes the split computable. The framework IS the structure generated by this single asymmetric naming act, read through three inseparable faces: production (R), observation (N), mediation (h = JN).
...
| family disc = 1+k^2 = framework quantities | COMPUTED (2,5,10,17,26 all identified) |
