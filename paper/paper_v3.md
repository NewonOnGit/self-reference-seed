...
**Theorem 24.4 (Semantic grounding).** Random initialization $\to$ trained on usage pairs only $\to$ all three sectors converge: PA 100%, MA 100%, OA 100% (from 23% random baseline). The algebra learns the semantic embedding from corpus. $R/N/h$ separation emerges from usage alone. Total: 207 automated tests.
...
207 automated tests. Two inputs. Zero free parameters.
